...
$queryYear = "SELECT EAU.*, CLTIP.program_name
              FROM EligibleAreaUpdates EAU
              JOIN CLTIP ON EAU.ARN = CLTIP.ARN
              WHERE YEAR(EAU.updated_at) = '$currentYear'
              AND CLTIP.program_name = 'CLTIP'";
...
<html lang="en">
<head>
    <?php $page_title = "Annual eligible areas CLTIP"; ?>
...
<body>
    ...
    <h1 class="section-title">Annual eligible areas Conservation Land Tax Incentive Program</h1>
    <?php
    if (!$hasEntriesYear) {
...
<form id="export_form_year" method="post" action="export-eligible.php">
    <input type="hidden" name="program_name" value="CLTIP" />
